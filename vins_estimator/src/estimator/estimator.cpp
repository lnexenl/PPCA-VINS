...
bool Estimator::getIMUInterval(double t0, double t1, vector<pair<double, Eigen::Vector3d>> &accVector,
    ...
    if (t1 <= accBuf.back().first) {
        while (accBuf.front().first <= t0) { // align the time.
            accBuf.pop();
            gyrBuf.pop();
        }
...
Estimator::processImage(const map<int, vector<pair<int, Eigen::Matrix<double, 7, 1>>>> &image, const double header) {
    ...
    if (solver_flag == INITIAL) // SOLVER_FLAG INITIAL requires enough frames to initialize.
    {
    ...
    } else // SOLVER_FLAG NONLINEAR, optimize when a new frame comes.
    {
        TicToc t_solve;
        if (!USE_IMU)
            f_manager.initFramePoseByPnP(frame_count, Ps, Rs, tic, ric);
