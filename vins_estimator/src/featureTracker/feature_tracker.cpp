    ...
    */
    ...
    if (prev_pts.size() > 0) {
        ...
        if (hasPrediction) {
            ...
            if (succ_num < 10) // if less than 10 pts' optical flow is successfully calculated.
                cv::calcOpticalFlowPyrLK(prev_img, cur_img, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
        } else // no predict.
            cv::calcOpticalFlowPyrLK(prev_img, cur_img, prev_pts, cur_pts, status, err, cv::Size(21, 21), 3);
        // reverse check
        if (FLOW_BACK) //flow back to reject some feature points. (eg. features that are moving on a line)
        {
    ...
    if (!_img1.empty() && stereo_cam) //stereo case
    {
        ids_right.clear();
        cur_right_pts.clear();
