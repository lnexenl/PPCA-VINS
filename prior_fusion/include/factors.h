...
#ifndef VINS_FACTORS_H
#define VINS_FACTORS_H
...
struct RelativeRError
{
    RelativeRError(double q_w, double q_x, double q_y, double q_z, double var)
        : q_w(q_w), q_x(q_x), q_y(q_y), q_z(q_z), var(var) {}

    template <typename T>
    bool operator()(const T* w_q_i, const T* w_q_j, T* residuals) const {
        T relative_q[4];
        relative_q[0] = T(q_w);
        relative_q[1] = T(q_x);
        relative_q[2] = T(q_y);
    ...
    static ceres::CostFunction* Create(const double q_w, const double q_x, const double q_y, const double q_z,
        ...
        return (new ceres::AutoDiffCostFunction<
                ...
                4,
                4>(
            new RelativeRError(q_w, q_x, q_y, q_z, var)));
    }
